...
#			genetic structure of Thamnophilus
#
...
# library(tidyr)
# library(tidyverse)
# setwd('~/AMNH/Roosevelt_Project/Data/CAs/Galbula/G_cy_DXY/')
# 
# #load and average 10,000 DXY replicates 
...
# }
# 
# DXY.raw<-t/length(files)
# write.csv(DXY.raw,file="../DXY.raw.csv")
# 
...
DXY.raw<-read.delim("T_ae_distances_12Jan2022.csv",header=T)
...
abline(v=-0.725,col="red",lwd=2,lty=2)
DXY[DXY> -0.725]<-1
DXY[DXY< -0.725]<-0
...
pdf('Thamno_CA_logistic_DXY.pdf',width=8,height=5, bg='transparent')
tmp = resBootstrap
tmp = round(tmp,3)
tmp$n = rev(tmp$n)
