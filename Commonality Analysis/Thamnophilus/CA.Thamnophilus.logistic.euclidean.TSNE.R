...
#			geographic distance (least cost-path) is used in place of dispersal (euclidiean) distance.  
...
#			genetic structure of Thamnophilus aethiops
#
...
# library(tidyverse)
# 
# setwd('~/AMNH/Roosevelt_Project/Data/CAs/Galbula/G_cy_TSNE/')
# 
# #load and average 10,000 TSNE replicates 
# files <- list.files(path="./", pattern="*.csv", full.names=F, recursive=FALSE)
# 
# tbl <- sapply(files, read_csv, simplify=FALSE)
# 
# t<-data.frame(matrix(0, nrow = 51, ncol = 51))
# 
...
setwd('~/AMNH/Roosevelt_Project/Data/CAs/Thamnophilus/')

TSNE.raw<-read.csv("TSNE.raw.csv",header=T)

#load coordinates and other files
data = read.csv('Indivs.Table.Thamnophilus.csv',stringsAsFactors=F)

#load barrier data
bar = read.delim('mbar.thamnophilus.txt')
...
abline(v=-1.275,col="red",lwd=2,lty=2)
TSNE[TSNE> -1.275]<-1
TSNE[TSNE< -1.275]<-0
...
for (i in c(1,2,3)){
  ...
  prGLM <- predict(glmt,newdata=Newdata,type="link",se=TRUE)
  plot(Zscore,lg(prGLM$fit),type="l",ylim=c(0,1),col="green",lty=1,lwd=2)
  par(new=T)}
}	
...
for (i in 1:nperm){
  ...
  mmTSNE[mmTSNE> -1.275]<-1
  mmTSNE[mmTSNE< -1.275]<-0
...
pdf('Thamno_CA_logistic_euclidean_TSNE.pdf',width=8,height=5, bg='transparent')
tmp = resBootstrap
tmp = round(tmp,3)
tmp$n = rev(tmp$n)
