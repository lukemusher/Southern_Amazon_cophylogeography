...
###   load barrier data
bar = read.delim('mbar.phleg.txt', stringsAsFactors = F)
...
abline(v=-1.675,col="red",lwd=2,lty=2)
#TSNE[TSNE> -1.675]<-1
#TSNE[TSNE< -1.675]<-0
...
for (i in 1:nperm){
  ...
  mmTSNE[mmTSNE> -1.675]<-1
  mmTSNE[mmTSNE< -1.675]<-0
...
pdf('Phleg_CA_logistic_cost_filtered_TSNE.pdf',width=8,height=5, bg='transparent')
tmp = resBootstrap
tmp = round(tmp,3)
tmp$n = rev(tmp$n)
