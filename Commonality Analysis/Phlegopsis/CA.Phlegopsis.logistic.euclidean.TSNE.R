...
#			genetic structure of Phlegopsis
#
...
setwd('~/AMNH/Roosevelt_Project/Data/CAs/Phlegopsis/')

TSNE.raw<-read.csv("TSNE.raw.csv",header=T)

#load coordinates and other files
data = read.csv('Indivs.Table.Phlegopsis.csv',stringsAsFactors=F)

#load barrier data
bar = read.delim('mbar.phleg.txt')
...
abline(v=-1.68,col="red",lwd=2,lty=2)
TSNE[TSNE> -1.68]<-1
TSNE[TSNE< -1.68]<-0
...
for (i in 1:nperm){
  ...
  mmTSNE[mmTSNE> -1.68]<-1
  mmTSNE[mmTSNE< -1.68]<-0
...
pdf('Phleg_CA_logistic_euclidean_TSNE.pdf',width=8,height=5, bg='transparent')
tmp = resBootstrap
tmp = round(tmp,3)
tmp$n = rev(tmp$n)
