...
#			genetic structure of Malacoptila
#
...
setwd('~/AMNH/Roosevelt_Project/Data/CAs/Malacoptila/')

TSNE.raw<-read.csv("TSNE.raw.csv",header=T)

#load coordinates and other files
data = read.csv('Indivs.Table.Malacoptila.csv',stringsAsFactors=F)

#load barrier data
bar = read.delim('mbar.malacoptila.txt')
...
abline(v=-1.7,col="red",lwd=2,lty=2)
TSNE[TSNE> -1.7]<-1
TSNE[TSNE< -1.7]<-0
...
pdf('Mala_CA_logistic_euclidean_TSNE.pdf',width=8,height=5, bg='transparent')
tmp = resBootstrap
tmp = round(tmp,3)
tmp$n = rev(tmp$n)
