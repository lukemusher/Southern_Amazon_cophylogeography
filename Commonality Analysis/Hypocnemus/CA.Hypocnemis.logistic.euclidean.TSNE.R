...
#			genetic structure of Hypocnemis
#
...
abline(v=-1.015,col="red",lwd=2,lty=2)
TSNE[TSNE> -1.015]<-1
TSNE[TSNE< -1.015]<-0
...
for (i in 1:nperm){
  ...
  mmTSNE[mmTSNE> -1.015]<-1
  mmTSNE[mmTSNE< -1.015]<-0
...
pdf('Hypoc_CA_logistic_euclidean_TSNE.pdf',width=8,height=5, bg='transparent')
tmp = resBootstrap
tmp = round(tmp,3)
tmp$n = rev(tmp$n)
