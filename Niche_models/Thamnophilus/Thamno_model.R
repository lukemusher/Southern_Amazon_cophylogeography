...
setwd("~/AMNH/Roosevelt_Project/Niche_models/Thamnophilus/")
...
#####################  Thamno  ##################################
######################################################################
## Thamno - thin points and select dataset
s1<-read.csv('./thamno_thinned_thin1.csv')[,2:3]
s2<-read.csv('./thamno_thinned_thin2.csv')[,2:3]
s3<-read.csv('./thamno_thinned_thin3.csv')[,2:3]
s4<-read.csv('./thamno_thinned_thin4.csv')[,2:3]
s5<-read.csv('./thamno_thinned_thin5.csv')[,2:3]
...
# load points
randomThin <- sample(1:5,1) # 3
Thamno<-read.csv('./thamno_thinned_thin3.csv')[,2:3]
...
buff.dist <- 5000
## Creating A MCP background for thinned Thamno data
ca.MCP<-mcp(Thamno)
...
shapefile(ca.shp, filename = "../../shapes/Thamno.shp", overwrite=TRUE)

ca.env.crp<-crop(bio, ca.shp)
ca.env<-mask(ca.env.crp, ca.shp)
plot(ca.env[[1]])
ca.backg<-randomPoints(ca.env, 10000)
write.csv(ca.backg, 'ThamnoBackg.csv', row.names = F)
caback<-read.csv('ThamnoBackg.csv')
...
cares<-ENMevaluate(occ = Thamno, env = ca.env, 
...
write.csv(cares@results, "ThamnoResults.csv", row.names=F)
Thamno_res<-read.csv('ThamnoResults.csv')
ThamnoOpt<-optimize(Thamno_res) # LQHP_5.5
#dir.create("ThamnoModel")
ThamnoMod<-maxent(x = ca.env,
                p = Thamno,
                a = caback,
                path = "ThamnoModel",
                args=c(
...
saveRDS(ThamnoMod, "ThamnoModel/ThamnoMaxent.RDS")
ThamnoMod<-readRDS("ThamnoModel/ThamnoMaxent.RDS")

## Predict current to full extent
bio<-stack(list.files(path = '~/AMNH/GIS_layers/Worldclim_bioclimatic/Current_wc2.1_30s_bio/', pattern = '\\.tif$', full.names = T))
colnames(Thamno) <- c("x","y")
e1 <- c(extent(Thamno)[1]-2, extent(Thamno)[2]+2, extent(Thamno)[3]-2, extent(Thamno)[4]+2)
bio <- crop(bio, e1)

ThamnoCurFull<-predict(
  object = ThamnoMod,
  x = bio,
  filename = "./ThamnoModel/CurrentThamnoFull.tif",
  na.rm=T,
  format = "GTiff",
  overwrite=T,
  args = 'cloglog'
...
ThamnoCurFull<-predict(
  object = ThamnoMod,
  x = LGM,
  filename = "./ThamnoModel/LGMThamnoFull.tif",
  na.rm=T,
  format = "GTiff",
  overwrite=T,
  args = 'cloglog'
