...
setwd("~/AMNH/Roosevelt_Project/Niche_models/Hypocnemis/")
...
#####################  Hypoc  ##################################
######################################################################
## Hypoc - thin points and select dataset
s1<-read.csv('./Hypoc_thinned_thin1.csv')[,2:3]
s2<-read.csv('./Hypoc_thinned_thin2.csv')[,2:3]
s3<-read.csv('./Hypoc_thinned_thin3.csv')[,2:3]
s4<-read.csv('./Hypoc_thinned_thin4.csv')[,2:3]
s5<-read.csv('./Hypoc_thinned_thin5.csv')[,2:3]
...
Hypoc<-read.csv('./Hypoc_thinned_thin5.csv')[,2:3]
...
buff.dist <- 5000
## Creating A MCP background for thinned Hypoc data
ca.MCP<-mcp(Hypoc)
...
shapefile(ca.shp, filename = "../../shapes/Hypoc.shp", overwrite=TRUE)

ca.env.crp<-crop(bio, ca.shp)
ca.env<-mask(ca.env.crp, ca.shp)
plot(ca.env[[1]])
ca.backg<-randomPoints(ca.env, 10000)
write.csv(ca.backg, 'HypocBackg.csv', row.names = F)
caback<-read.csv('HypocBackg.csv')
...
cares<-ENMevaluate(occ = Hypoc, env = ca.env, 
...
write.csv(cares@results, "HypocResults.csv", row.names=F)
Hypoc_res<-read.csv('HypocResults.csv')
HypocOpt<-optimize(Hypoc_res) # H_6
#dir.create("HypocModel")

HypocMod<-maxent(x = ca.env,
                  p = Hypoc,
                  a = caback,
                  path = "HypocModel",
                  args=c(
...
saveRDS(HypocMod, "HypocModel/HypocMaxent.RDS")
HypocMod<-readRDS("HypocModel/HypocMaxent.RDS")

## Predict current to full extent
bio<-stack(list.files(path = '~/AMNH/GIS_layers/Worldclim_bioclimatic/Current_wc2.1_30s_bio/', pattern = '\\.tif$', full.names = T))
colnames(Hypoc) <- c("x","y")
e1 <- c(extent(Hypoc)[1]-2, extent(Hypoc)[2]+2, extent(Hypoc)[3]-2, extent(Hypoc)[4]+2)
bio <- crop(bio, e1)

HypocCurFull<-predict(
  object = HypocMod,
  x = bio,
  filename = "./HypocModel/CurrentHypocFull.tif",
  na.rm=T,
  format = "GTiff",
  overwrite=T,
  args = 'cloglog'
...
HypocCurFull<-predict(
  object = HypocMod,
  x = LGM,
  filename = "./HypocModel/LGMHypocFull.tif",
  na.rm=T,
  format = "GTiff",
  overwrite=T,
  args = 'cloglog'
