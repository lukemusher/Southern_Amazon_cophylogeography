...
setwd("~/AMNH/Roosevelt_Project/Niche_models/Willisornis/")
...
#####################  Willis  ##################################
######################################################################
## Willis - thin points and select dataset
s1<-read.csv('./Willis_thinned_thin1.csv')[,2:3]
s2<-read.csv('./Willis_thinned_thin2.csv')[,2:3]
s3<-read.csv('./Willis_thinned_thin3.csv')[,2:3]
s4<-read.csv('./Willis_thinned_thin4.csv')[,2:3]
s5<-read.csv('./Willis_thinned_thin5.csv')[,2:3]
...
# load points
randomThin <- sample(1:5,1) # 5
Willis<-read.csv('./Willis_thinned_thin5.csv')[,2:3]
...
buff.dist <- 5000
## Creating A MCP background for thinned Willis data
ca.MCP<-mcp(Willis)
...
shapefile(ca.shp, filename = "../../shapes/Willis.shp", overwrite=TRUE)

ca.env.crp<-crop(bio, ca.shp)
ca.env<-mask(ca.env.crp, ca.shp)
plot(ca.env[[1]])
ca.backg<-randomPoints(ca.env, 10000)
write.csv(ca.backg, 'WillisBackg.csv', row.names = F)
caback<-read.csv('WillisBackg.csv')
...
cares<-ENMevaluate(occ = Willis, env = ca.env, 
...
write.csv(cares@results, "WillisResults.csv", row.names=F)
Willis_res<-read.csv('WillisResults.csv')
WillisOpt<-optimize(Willis_res) # H_6
#dir.create("WillisModel")

WillisMod<-maxent(x = ca.env,
                  p = Willis,
                  a = caback,
                  path = "WillisModel",
                  args=c(
...
saveRDS(WillisMod, "WillisModel/WillisMaxent.RDS")
WillisMod<-readRDS("WillisModel/WillisMaxent.RDS")

## Predict current to full extent
bio<-stack(list.files(path = '~/AMNH/GIS_layers/Worldclim_bioclimatic/Current_wc2.1_30s_bio/', pattern = '\\.tif$', full.names = T))
colnames(Willis) <- c("x","y")
e1 <- c(extent(Willis)[1]-2, extent(Willis)[2]+2, extent(Willis)[3]-2, extent(Willis)[4]+2)
bio <- crop(bio, e1)

WillisCurFull<-predict(
  object = WillisMod,
  x = bio,
  filename = "./WillisModel/CurrentWillisFull.tif",
  na.rm=T,
  format = "GTiff",
  overwrite=T,
  args = 'cloglog'
...
WillisCurFull<-predict(
  object = WillisMod,
  x = LGM,
  filename = "./WillisModel/LGMWillisFull.tif",
  na.rm=T,
  format = "GTiff",
  overwrite=T,
  args = 'cloglog'
