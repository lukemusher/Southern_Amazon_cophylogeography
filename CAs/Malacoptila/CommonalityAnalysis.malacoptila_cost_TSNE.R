...
setwd('~/AMNH/Roosevelt_Project/Data/CAs/Malacoptila/M_ru_TSNE/')
files <- list.files(path="./", pattern="*.csv", full.names=F, recursive=FALSE)

tbl <- sapply(files, read_csv, simplify=FALSE)

t<-data.frame(matrix(0, nrow = 50, ncol = 50))
...
pdf('Malacoptila_CA_TSNE.pdf',width=8,height=5, bg='transparent')
tmp = resBootstrap
tmp = round(tmp,3)
tmp$n = rev(tmp$n)
...
setwd("~/AMNH/Roosevelt_Project/Data/CAs/Malacoptila/")
bar2 = read.delim('./mbar.malacoptila.txt')
bar2[bar2==1]<-"River"
...
plot(as.factor(bar2.vec),TSNE.vec,ylab="Genomic Divergence in TSNE space",xlab="", main="Malacoptila")
