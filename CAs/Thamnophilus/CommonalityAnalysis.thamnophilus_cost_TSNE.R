...
#r = raster('~/AMNH/Roosevelt_Project/Niche_models/Thamnophilus/ThamnoModel/CurrentThamnoFull.tif')
#rnew = r
...
setwd('~/AMNH/Roosevelt_Project/Data/CAs/Thamnophilus/T_ae_TSNE/')
files <- list.files(path="./", pattern="*.csv", full.names=F, recursive=FALSE)

tbl <- sapply(files, read_csv, simplify=FALSE)

t<-data.frame(matrix(0, nrow = 61, ncol = 61))
...
pdf('Thamnophilus_CA_TSNE.pdf',width=8,height=5, bg='transparent')
tmp = resBootstrap
tmp = round(tmp,3)
tmp$n = rev(tmp$n)
