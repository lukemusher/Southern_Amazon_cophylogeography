...
r = raster('~/AMNH/Roosevelt_Project/Niche_models/Willisornis/WillisModel/CurrentWillisFull.tif')
#rnew = r
...
setwd('~/AMNH/Roosevelt_Project/Data/CAs/Willisornis/W_po_TSNE/')
files <- list.files(path="./", pattern="*.csv", full.names=F, recursive=FALSE)

tbl <- sapply(files, read_csv, simplify=FALSE)

t<-data.frame(matrix(0, nrow = 73, ncol = 73))
...
pdf('Willisornis_CA_TSNE.pdf',width=8,height=5, bg='transparent')
tmp = resBootstrap
tmp = round(tmp,3)
tmp$n = rev(tmp$n)
...
setwd("~/AMNH/Roosevelt_Project/Data/CAs/Willisornis/")
bar2 = read.delim('./mbar.Willisornis.txt')
bar2[bar2==1]<-"River"
bar2[bar2==0]<-"No River"

count=0
bar2.vec<-c()
for(i in 1:length(bar2[1,])){
  for (j in 1:length(bar2[,1])){
    count=count+1
    bar2.vec[count]<-(bar2[i,j])
  }
}
...
for(i in 1:length(bar2[1,])){
  for (j in 1:length(bar2[,1])){
    count=count+1
    cost.vec[count]<-(cost.raw[i,j])
  }
}


plot(as.factor(bar2.vec),TSNE.vec,ylab="Genomic Divergence in TSNE space",xlab="", main="Willisornis")
plot(cost.vec,TSNE.vec)
abline(lm(TSNE.vec~cost.vec),col='red',lwd=2)

summary(glm(TSNE.vec~cost.vec))
summary(glm(TSNE.vec~cost.vec*bar2.vec))
